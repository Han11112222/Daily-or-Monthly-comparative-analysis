...
# ─────────────────────────────────────────────
# 3. 데이터 로딩 (형님 코드 로직 + 자동 탐색)
# ─────────────────────────────────────────────
@st.cache_data(show_spinner=False)
def load_monthly_plan(uploaded_file):
    """월별 계획 로딩: 연도 컬럼을 찾지 않고, 월 컬럼만 인식"""
    df = None
    # 1. 업로드 확인
...
def load_daily_data(uploaded_file_daily):
    ...
    if "공급량(MJ)" in df.columns:
        df["공급량(MJ)"].apply(pd.to_numeric, errors="coerce")
...
def _add_cumulative_sheet(wb, target_year):
    ...
    # 수식 예시 (B4셀 등)
    d = "$B$1"
    ws["B4"] = f'=IFERROR(XLOOKUP({d},연간!$A:$A,연간!$F:$F),"")' # 예시 수식
    # ... (나머지 수식은 공간상 생략하나 로직은 동일하게 적용됨)
...
def main():
    st.sidebar.title("설정")
    up_daily = st.sidebar.file_uploader("일일 실적(선택)", type=["xlsx"])
    
    # 1. 일일 데이터 로드
    df_daily = load_daily_data(up_daily)
    if df_daily is None:
        st.warning("⚠️ '공급량(일일실적).xlsx' 파일이 없습니다. 폴더를 확인해주세요.")
        return
    ...
    if tab == "Daily 공급량 분석":
        ...
        up_plan = st.file_uploader("월별 계획(선택)", type=["xlsx"])
        df_plan_month = load_monthly_plan(up_plan)
        
        if df_plan_month is None:
            st.warning("⚠️ '월별계획.xlsx' 파일이 없습니다.")
            return

        # 2. 연도 선택 (중요: 계획 파일이 아니라 실적 파일에서 연도 추출)
        ...
        m_total_gj = float(df_plan_month.loc[0, m_col]) # 첫 행의 해당 월 값 가져오기
