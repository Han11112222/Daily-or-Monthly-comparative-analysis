...
def load_monthly_plan(uploaded_file) -> pd.DataFrame:
    ...
    if uploaded_file is not None:
        ...
        # 2. 자동 탐색 (형님 파일명 기준)
        file_path = find_repo_file(["월별계획.xlsx", "월별 계획.xlsx"])
        if file_path:
            df = pd.read_excel(file_path)
        else:
            return None # 에러 안 내고 None 반환
...
def make_daily_plan_table(df_daily, target_year, target_month, monthly_total_gj, n_years=3):
    ...
    for y in cand_years:
        ...
        if not sub.empty and sub["공급량(MJ)"].notna().sum() > 0:
            used_years.append(y)
            df_hist.append(sub)
...
def main():
    ...
    if df_daily is None:
        ...
        st.info("깃허브 레포지토리에 파일이 있는지 확인하거나, 좌측에서 파일을 업로드해주세요.")
        return
    ...
    if tab == "Daily 공급량 분석":
        ...
        uploaded_plan = st.sidebar.file_uploader("월별 계획(선택사항)", type=["xlsx"])
        ...
        st.info(f"**{t_year}년 {t_month}월 목표**: {m_total_gj:,.0f} GJ (자동 인식됨)")

        # 분석 실행
        df_res, used_yrs = make_daily_plan_table(df_daily, t_year, t_month, m_total_gj, n_yrs)
        
        if df_res is not None:
            st.success(f"✅ 학습에 사용된 연도: {used_yrs}")
            ...
            if c_d2.button("📥 연간 전체 생성(누적현황 포함)"):
                ...
                for m in range(1, 13):
                    mc = None
                    for cand in [f"{m}월", str(m), f"{m:02d}"]:
                        if cand in df_plan_month.columns: mc = cand
